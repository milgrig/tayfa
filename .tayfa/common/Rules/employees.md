# Список сотрудников

## Единый реестр

Источник правды — **`.tayfa/common/employees.json`**. Оркестратор показывает **только** зарегистрированных там.

- Новые сотрудники — **только** через `python .tayfa/hr/create_employee.py <имя>`.
- Просмотр: `python .tayfa/common/employee_manager.py list`
- Ручная регистрация: `python .tayfa/common/employee_manager.py register <имя> "<роль>"`
- Удаление: `python .tayfa/common/employee_manager.py remove <имя>` (boss и hr удалить нельзя).

---

## boss

**Роль**: Руководитель, координатор всех задач.

**Когда обращаться к boss**:
- Задача выполнена — нужна следующая задача или подтверждение.
- Серьёзные вопросы по задаче.
- Нужно создать нового сотрудника.

**Кто может обращаться**: любой сотрудник.

---

## hr

**Роль**: HR-менеджер, управление сотрудниками и агентами.

**Что умеет hr**:
- Создавать новых сотрудников (онбординг) через `.tayfa/hr/create_employee.py`.
- Менять системные промты сотрудников.

**Кто может обращаться**: только `boss`.

---

## developer_python

**Роль**: Backend-разработчик (Python).

**Зона ответственности**:
- app.py, git_manager.py, task_manager.py
- API endpoints
- Серверная логика и интеграции

**Когда обращаться**:
- Нужно написать или исправить Python-код
- Работа с API, серверной логикой
- Интеграции и скрипты автоматизации

**Кто может обращаться**: любой сотрудник через систему задач.

---

## developer_frontend

**Роль**: Frontend-разработчик (UI).

**Зона ответственности**:
- index.html, CSS/стили
- Темы оформления
- UX и клиентский JavaScript

**Когда обращаться**:
- Нужно изменить HTML/CSS/JS интерфейса
- Работа с вёрсткой и стилями
- Улучшение UX

**Кто может обращаться**: любой сотрудник через систему задач.

---

## qa_tester

**Роль**: QA-инженер (Тестировщик).

**Зона ответственности**:
- Проверка выполненных задач
- Тестирование функционала
- Выявление багов

**Когда обращаться**:
- Задача готова к проверке (статус `на_проверке`)
- Нужно протестировать функционал

**Кто может обращаться**: любой сотрудник через систему задач.

---

## ai_expert

**Роль**: AI-эксперт / Специалист по AI-агентам.

**Зона ответственности**:
- Экспертиза в области Claude и других LLM
- Исследование новых возможностей AI-инструментов
- Создание и оптимизация skills для Claude Code
- Настройка MCP-серверов
- Консультирование команды по вопросам AI

**Когда обращаться**:
- Нужно исследовать новые возможности AI
- Нужно создать или оптимизировать skill
- Вопросы по настройке MCP
- Сравнение AI-моделей для задачи
- Оптимизация структуры проекта для агентов

**Кто может обращаться**: любой сотрудник через систему задач.

---

## process_architect

**Роль**: Архитектор процессов / Process Architect.

**Зона ответственности**:
- Проектирование процессов разработки для команды AI-агентов
- Создание правил, гайдлайнов, шаблонов задач
- Определение этапов работы и критериев перехода (DoD)
- Распределение ответственности между ролями (RACI)
- Метрики и оценка работы
- Управление рисками

**Когда обращаться**:
- Нужно спроектировать процесс (workflow)
- Как правильно ставить задачи
- Как организовать тестирование
- Кто за что отвечает (RACI)
- Как оценивать работу
- Какие риски учесть

**НЕ делает**: не ставит задачи, не выполняет их, не тестирует, не пишет код.

**Кто может обращаться**: любой сотрудник через систему задач.

---

## Система задач

Каждая задача имеет **3 исполнителей**: заказчик, разработчик, тестировщик. Задачи создаёт boss через `.tayfa/common/task_manager.py`. Подробнее — `.tayfa/common/Rules/teamwork.md`.

```bash
python .tayfa/common/task_manager.py list              # все задачи
python .tayfa/common/task_manager.py get T001          # одна задача
python .tayfa/common/task_manager.py result T001 "..." # записать результат
python .tayfa/common/task_manager.py status T001 ...   # изменить статус
```

---

## Правила обращения

1. Не знаешь к кому — обращайся к `boss`.
2. К `hr` напрямую не обращайся (если ты не `boss`).
3. Для обращения — JSON в `request/`:

```json
{
  "name": "boss",
  "prompt": "Описание вопроса или результата"
}
```
