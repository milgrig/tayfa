...
def _get_system_user_data_dir() -> "Path | None":
    """Resolve the system-level Tayfa user data directory.

    Windows:  %APPDATA%\\Tayfa
    Linux:    ~/.config/tayfa
    macOS:    ~/Library/Application Support/Tayfa
    Override: TAYFA_USER_DATA env var.
    """
    import os as _os, platform as _pf
    env = _os.environ.get("TAYFA_USER_DATA")
    if env:
        p = Path(env)
    ...
    if system == "Windows":
        ...
        p = Path.home() / "Library" / "Application Support" / "Tayfa"
        return p if p.is_dir() else None
    else:
...
def _find_user_file(filename: str, project_root: "Path | None") -> "Path | None":
    """Locate a user data file (settings.json, secret_settings.json, projects.json).

    Search order:
    1. System-level user data dir (%APPDATA%/Tayfa)
    2. {tayfa_repo}/kok/{filename}  (legacy — file inside the repo)
    3. Scan parent directories for sibling dirs with kok/{filename}
    """
    # 1. System-level (primary)
    sys_dir = _get_system_user_data_dir()
    if sys_dir:
        p = sys_dir / filename
        if p.exists():
            return p

    # 2. Legacy: inside kok/ of the Tayfa repo (or project root)
    if project_root:
        p = project_root / "kok" / filename
        if p.exists():
            return p

    # 3. Scan parent directories for sibling Tayfa repos
    if project_root:
...
def _get_github_token() -> str:
    """Get GitHub token from secret_settings.json."""
    project_root = _get_project_root()
    secret_path = _find_user_file("secret_settings.json", project_root)
    if not secret_path:
        return ""
    try:
...
def _get_github_owner() -> str:
    """Get GitHub owner from settings.json."""
    project_root = _get_project_root()
    settings_path = _find_user_file("settings.json", project_root)
...
def _get_repo_name() -> str:
    """Get repo name from projects.json for the current project.
    ...
    if not project_root:
        return ""
    projects_path = _find_user_file("projects.json", project_root)
