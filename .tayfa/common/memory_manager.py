...
if __name__ == "__main__":
    ...
    # Auto-detect .tayfa dir: this file lives in .tayfa/common/
    _this_file = Path(__file__).resolve()
    _auto_tayfa = _this_file.parent.parent  # .tayfa/common -> .tayfa
    if _auto_tayfa.name == ".tayfa":
        set_tayfa_dir(_auto_tayfa)

    if len(sys.argv) < 2:
        print("Usage:")
        print("  python memory_manager.py save <agent_name> \"<summary>\"")
        print("  python memory_manager.py read <agent_name>")
        sys.exit(1)
    ...
    if cmd == "save" and len(sys.argv) >= 4:
        agent = sys.argv[2]
        summary = sys.argv[3]
        ok = update_memory(agent, "note", summary)
        if ok:
            print(f"Saved to {agent}/memory.md")
        else:
            print(f"Error: could not write memory for {agent}", file=sys.stderr)
            sys.exit(1)

    elif cmd == "read" and len(sys.argv) >= 3:
        agent = sys.argv[2]
        content = build_memory(agent)
        if content:
            print(content)
        else:
            print(f"(no memory for {agent})")
