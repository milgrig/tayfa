...
async def api_trigger_task(task_id: str, data: dict = Body(default_factory=dict)):
    ...
    # Route to the correct runtime based on model
    if is_ollama_model(resolved_model):
        runtime = "ollama"
    elif is_cursor_model(resolved_model):
        runtime = "cursor"
            ...
            for attempt in range(1, _MAX_RETRY_ATTEMPTS + 1):
                ...
                try:
                    if runtime == "ollama":
                        ...
                        from routers.agents import compose_system_prompt
                        system_prompt = compose_system_prompt(agent_name) or ""
                        result = await ollama_provider.call_ollama(
                            resolved_model, full_prompt, system_prompt,
                        )
                        duration_sec = _time.time() - start_time

                        if result.get("is_error"):
                            raise HTTPException(status_code=502, detail=f"Ollama error: {result.get('error', 'unknown')}")
                        ...
                        save_chat_message(
                            ...
                            task_id=task_id,
                            success=True,
                            extra={"role": role_label, **(result.get("ollama_usage") or {})},
                        )
