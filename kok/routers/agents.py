...
from app_state import (
    ...
    _MODEL_RUNTIMES, is_cursor_model, is_ollama_model,
    _debug_log_ensure,
    estimate_tokens,
    logger,
)
...
def _find_native_agent() -> str | None:
    """Find native Windows cursor-agent executable (no WSL needed).
    Checks %LOCALAPPDATA%\\cursor-agent first, then PATH."""
    ...
    if localappdata:
        for name in ("agent.cmd", "cursor-agent.cmd"):
            ...
            if os.path.isfile(p):
                return p
    return _shutil.which("agent") or _shutil.which("cursor-agent")
...
def _resolve_cursor_model(model: str | None) -> str:
    """Resolve model string: apply legacy aliases, fall back to CURSOR_CLI_MODEL."""
    raw = (model or CURSOR_CLI_MODEL).strip()
    return _CURSOR_MODEL_ALIASES.get(raw.lower(), raw)


# ── WSL fallback helpers (used only when native agent is not available) ────
...
def _to_wsl_path(path) -> str:
    """Converts a Windows path to WSL format. Used only for Cursor CLI via WSL."""
    p = str(path).replace("\\", "/")
    if p.startswith("/mnt/"):
        return p
    if len(p) >= 2 and p[1] == ":":
...
def _cursor_cli_base_script(workdir: str | None = None) -> str:
    """Base prefix for Cursor CLI commands in WSL: PATH and cd to project workdir."""
    target = workdir or get_agent_workdir()
    wsl_target = _to_wsl_path(target)
    return (
...
def _cursor_chat_key(agent_name: str, project_path: str = "", model: str = "") -> str:
    """Build composite key for .cursor_chats.json: '{project}:{agent}:{model}'.
    Ensures agents are isolated per project and per model."""
    parts = []
    if project_path:
...
def _parse_chat_id(out_text: str, returncode: int) -> str | None:
    """Extract chat_id from create-chat output (JSON or plain UUID)."""
    if not out_text or returncode != 0:
        return None
    chat_id = None
    try:
...
async def run_cursor_cli_create_chat(workdir: str | None = None) -> dict:
    """
    Creates a chat in Cursor CLI: agent --print --output-format json create-chat.
    Uses native Windows agent if available, falls back to WSL.
    Returns { "success": bool, "chat_id": str | None, "raw": str, "stderr": str }.
    """
    resolved_workdir = workdir or get_agent_workdir()
    try:
        if _NATIVE_AGENT:
            cmd = [_NATIVE_AGENT, "--print", "--output-format", "json", "create-chat"]
            ...
            wsl_script = (
                f"{_cursor_cli_base_script(resolved_workdir)} && "
        ...
        return {"success": False, "chat_id": None, "raw": "", "stderr": f"Timeout {CURSOR_CREATE_CHAT_TIMEOUT} sec."}
    except Exception as e:
        return {"success": False, "chat_id": None, "raw": "", "stderr": str(e)}

    chat_id = _parse_chat_id(out_text, proc.returncode)
    return {
...
def _build_cursor_cli_prompt(agent_name: str, user_prompt: str) -> str:
    """Builds prompt for Cursor CLI: role (agent name) + project context + task."""
...
async def ensure_cursor_chat(agent_name: str, project_path: str = "", model: str = "") -> tuple[str | None, str]:
    """
    Returns (chat_id, error): chat_id for agent from .cursor_chats.json or creates a new one (create-chat).
    Uses composite key (project:agent:model) for isolation between projects and models.
    On error: (None, error_message).
    """
    key = _cursor_chat_key(agent_name, project_path, model)
    chats = _load_cursor_chats()
    if key in chats and chats[key]:
        return chats[key], ""
    workdir = project_path or get_agent_workdir()
    create_result = await run_cursor_cli_create_chat(workdir=workdir)
...
async def run_cursor_cli(
    ...
    """
    Runs Cursor CLI in headless mode. Uses native Windows agent if available, falls back to WSL.
    ...
    chat_error = ""
    if use_chat:
        chat_id, chat_error = await ensure_cursor_chat(agent_name, project_path=project_path, model=model)
    ...
    resolved_model = _resolve_cursor_model(model)
    resolved_workdir = project_path or get_agent_workdir()
...
async def run_cursor_cli_stream(
    ...
    """
    Runs Cursor CLI with --output-format stream-json.
    Uses native Windows agent if available, falls back to WSL.
...
async def send_prompt_cursor(data: dict):
    ...
    start_time = _time.time()
    result = await run_cursor_cli(name, prompt_text, use_chat=use_chat, model=cursor_model, project_path=pp)
    duration_sec = _time.time() - start_time
...
async def send_prompt_cursor_stream(data: dict):
    ...
    async def sse_generator():
        ...
        try:
            async for event in run_cursor_cli_stream(name, prompt_text, use_chat=use_chat, model=cursor_model, project_path=pp):
                etype = event.get("type", "")
...
async def ollama_models():
    ...
    available = await ollama_provider.check_available()
    if not available:
...
@router.post("/api/send-prompt-ollama")
async def send_prompt_ollama(data: dict):
    """Send a prompt to an Ollama local model (non-streaming). Saves history."""
    agent_name = data.get("name") or data.get("agent")
    prompt_text = data.get("prompt", "")
    task_id = data.get("task_id")
    model = data.get("model") or data.get("runtime") or "qwen2.5-coder:14b"

    if not agent_name or not prompt_text:
        raise HTTPException(400, "Fields 'name' and 'prompt' are required")

    system_prompt = compose_system_prompt(agent_name) or ""
    start_time = _time.time()
    result = await ollama_provider.call_ollama(model, prompt_text, system_prompt)
    duration_sec = _time.time() - start_time
    ...
    if agent_name and prompt_text:
        save_chat_message(
            ...
            extra=result.get("ollama_usage"),
        )

    _maybe_forward_reply_to_telegram(agent_name, result.get("result", ""))
    return result


@router.post("/api/send-prompt-ollama-stream")
async def send_prompt_ollama_stream(data: dict):
    """Send a prompt to an Ollama local model with SSE streaming. Saves history on completion."""
    agent_name = data.get("name") or data.get("agent")
    prompt_text = data.get("prompt", "")
    task_id = data.get("task_id")
    model = data.get("model") or data.get("runtime") or "qwen2.5-coder:14b"
    ...
    async def sse_generator():
        full_result = ""
        async for chunk_json in ollama_provider.stream_ollama(model, prompt_text, system_prompt):
...
async def cursor_create_chat(data: dict):
    ...
    Chat_id is saved to .cursor_chats.json with composite key (project:agent:model) and used for sending (--resume).
    """
    name = data.get("name") or data.get("agent")
    if not name:
    ...
    pp = get_project_path_for_scoping()
    emp = get_employee(name)
    cursor_model = data.get("model") or (emp or {}).get("model") or CURSOR_CLI_MODEL
    create_result = await run_cursor_cli_create_chat(workdir=pp or None)
    ...
    chat_id = create_result["chat_id"]
    key = _cursor_chat_key(name, pp, cursor_model)
    chats = _load_cursor_chats()
    chats[key] = chat_id
    _save_cursor_chats(chats)
...
async def cursor_create_chats():
    ...
    with composite key (project:agent:model).
    """
    chats = _load_cursor_chats()
    employees = _get_employees()
    pp = get_project_path_for_scoping()
    ...
    for agent_name in agents_with_cursor:
        emp_data = employees.get(agent_name, {})
        agent_model = emp_data.get("model") or CURSOR_CLI_MODEL
        key = _cursor_chat_key(agent_name, pp, agent_model)
        if key in chats and chats[key]:
            results.append({"agent": agent_name, "chat_id": chats[key], "created": False})
            continue
        create_result = await run_cursor_cli_create_chat(workdir=pp or None)
