...
async def api_register_employee(data: dict):
    ...
    После успешного создания автоматически вызывает ensure_agents() для
    создания агента в оркестраторе (T035: автоматическое обеспечение агентов).
    """
    name = data.get("name")
    role = data.get("role", "")
    ...
    result = register_employee(name, role, model)

    # T035: Автоматическое обеспечение агентов
    # После успешного создания сотрудника — автоматически создаём агента
    if result.get("status") == "created":
        try:
            ensure_result = await ensure_agents()
            result["agent_ensured"] = True
            result["ensure_details"] = ensure_result
        except Exception as e:
            # Если ensure_agents не удался — сотрудник создан, но агент не создан
            # Пользователь может нажать "Обеспечить агентов" вручную
            result["agent_ensured"] = False
            result["ensure_error"] = str(e)
