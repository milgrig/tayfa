...
Агенты взаимодействуют через **систему задач** (`common/task_manager.py`). Boss создаёт задачу с указанием постановщика, разработчика и тестировщика. Оркестратор на каждом этапе запускает соответствующего агента.
