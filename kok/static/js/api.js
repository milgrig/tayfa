...
let _cursorCliReady = false;

function updateCursorHeaderDot(loggedIn) {
    _cursorCliReady = loggedIn;
    const dot = document.getElementById('cursorDot');
    const label = document.getElementById('cursorStatus');
    if (dot) dot.className = `status-dot ${loggedIn ? 'on' : 'off'}`;
    if (label) label.textContent = loggedIn ? 'ready' : 'off';
}

function updateStatusUI(s) {
    const claudeReady = !!s.api_running;
    const claudeStarting = s.claude_api_running && !s.api_running;

    const claudeDot = document.getElementById('claudeDot');
    const claudeLabel = document.getElementById('claudeStatus');
    if (claudeDot) claudeDot.className = `status-dot ${claudeReady ? 'on' : claudeStarting ? 'loading' : 'off'}`;
    if (claudeLabel) claudeLabel.textContent = claudeReady ? 'ready' : claudeStarting ? 'starting' : 'off';

    // Cursor dot from cached CLI login status (updated by loadCursorModels / loadCliToolsStatus)
    if (s.cursor_cli_ready !== undefined) {
        updateCursorHeaderDot(s.cursor_cli_ready);
    }
...
async function startServer() {
    ...
    document.getElementById('claudeDot').className = 'status-dot loading';
    try {
