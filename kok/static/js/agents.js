...
// Ollama local model prefixes — routed to /api/send-prompt-ollama-stream
const _OLLAMA_PREFIXES = [
...
function _isOllamaModel(model) {
    if (!model) return false;
    const m = model.toLowerCase();
    if (m.startsWith('ollama:')) return true;
    return _OLLAMA_PREFIXES.some(p => m.startsWith(p));
}
...
function _modelFamily(model) {
    ...
    if (_isOllamaModel(m)) return 'ollama';
...
function _modelDisplayLabel(model) {
    ...
    const m = model.toLowerCase();
    if (_isOllamaModel(m)) {
        const clean = m.startsWith('ollama:') ? m.slice(7) : m;
        return '⚡ ' + clean;
    }
...
function _modelIsComposer(model) {
    return model && !_CLAUDE_API_MODELS.has(model) && !_isOllamaModel(model);
}

function isAgentOllama(agentName) {
    return _isOllamaModel(getAgentRuntime(agentName));
}
